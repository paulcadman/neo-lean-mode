;;; neo-lean-input-test.el --- Tests for neo-lean-input  -*- lexical-binding: t; -*-
...
;;
;; Run with:
;;   emacs -Q --batch -L . -l test/neo-lean-input-test.el \
...
(ert-deftest neo-lean-input-translation-plain ()
  "A marker-free expansion is returned unchanged and unpropertized."
  (let ((s (neo-lean-input--translation "α")))
    (should (equal s "α"))
    (should-not (get-text-property 0 'advice s))))

(ert-deftest neo-lean-input-translation-cursor-strips-marker ()
  "The $CURSOR marker is removed from the inserted text."
  (should (equal (substring-no-properties
                  (neo-lean-input--translation "⟨$CURSOR⟩"))
                 "⟨⟩")))

(ert-deftest neo-lean-input-translation-cursor-advice-moves-point ()
  "The advice closure leaves point where $CURSOR was."
  (let ((s (neo-lean-input--translation "⟨$CURSOR⟩")))
    (with-temp-buffer
      (insert s)
...
(ert-deftest neo-lean-input-translation-cursor-multichar-suffix ()
...
(ert-deftest neo-lean-input-translation-trailing-marker ()
  "A $CURSOR at the very end yields no advice (offset 0)."
  (let ((s (neo-lean-input--translation "foo$CURSOR")))
    (should (equal s "foo"))
    (should-not (get-text-property 0 'advice s))))

;;;; Building the method from real data

(ert-deftest neo-lean-input-setup-builds-package ()
  "`neo-lean-input-setup' registers the package and known rules."
  (neo-lean-input-setup)
  (should (assoc neo-lean-input-method-name quail-package-alist))
  (let ((quail-current-package
         (assoc neo-lean-input-method-name quail-package-alist)))
...
(provide 'neo-lean-input-test)
;;; neo-lean-input-test.el ends here
