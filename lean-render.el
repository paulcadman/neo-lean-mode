;;; lean-render.el --- Render interactive goals to text  -*- lexical-binding: t; -*-
...
(defconst lean-render-default-goal-prefix "⊢ "
  "Symbol shown before a goal's target type when none is provided.")

(defun lean-render-tagged-text (tt)
...
Tag payloads are ignored for now; only text leaves are kept."
  (cond
   ...
   ((plist-member tt :append)
    (mapconcat #'lean-render-tagged-text (plist-get tt :append) ""))
   ((plist-member tt :tag)
    ;; (:tag [PAYLOAD INNER]) -- render only the inner TaggedText.
    (lean-render-tagged-text (seq-elt (plist-get tt :tag) 1)))
   (t "")))

(defun lean-render--hypothesis (hyp)
...
their value after \":=\"."
  (let* ((names (string-join (append (plist-get hyp :names) nil) " "))
         (type (lean-render-tagged-text (plist-get hyp :type)))
         (val (plist-get hyp :val))
         (base (format "%s : %s" names type)))
    (if val
        (format "%s := %s" base (lean-render-tagged-text val))
      base)))

(defun lean-render-goal (goal)
  "Render one interactive GOAL to a string.
Shows an optional case name, the hypotheses, and the target type prefixed
by the goal's `goalPrefix' (default `lean-render-default-goal-prefix')."
  (let ((lines '())
        (user-name (plist-get goal :userName))
        (prefix (or (plist-get goal :goalPrefix) lean-render-default-goal-prefix)))
    (when (and user-name (not (string-empty-p user-name)))
      (push (format "case %s" user-name) lines))
    (seq-doseq (hyp (plist-get goal :hyps))
      (push (lean-render--hypothesis hyp) lines))
    (push (concat prefix (lean-render-tagged-text (plist-get goal :type))) lines)
    (string-join (nreverse lines) "\n")))

(defun lean-render-goals (goals)
  "Render the vector of interactive GOALS to a string.
Returns \"No goals.\" when GOALS is empty."
  (if (seq-empty-p goals)
      "No goals."
    (mapconcat #'lean-render-goal goals "\n\n")))

(provide 'lean-render)
;;; lean-render.el ends here
