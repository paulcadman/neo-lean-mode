;;; lean-mode.el --- Major mode for Lean 4  -*- lexical-binding: t; -*-
...
;; with lean.nvim.  This file defines `lean-mode' and configures Eglot to
...
;; of the current milestone is the interactive RPC layer (see `lean-rpc.el')
;; and the goal display (`lean-goal').
...
(defcustom lean-server-command '("lake" "serve")
...
;; `lean--eglot-managed-setup').
(add-to-list 'eglot-server-programs
             (cons 'lean-mode
                   (lambda (&optional _interactive _project)
                     lean-server-command)))

(defun lean--eglot-managed-setup ()
...
(add-hook 'eglot-managed-mode-hook #'lean--eglot-managed-setup)
